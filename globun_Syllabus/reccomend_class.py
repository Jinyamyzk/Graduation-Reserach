...
def test_desu():
    class_names_topics = df[['科目名','トピックの確率']].values.tolist()
    for class_name_topic in class_names_topics:
        combined = [x*y for (x,y) in zip(class_name_topic[1],sum_topic_odds)]
        total_class_score = sum(combined)
        class_name_topic[1] = total_class_score
        test.append(class_name_topic)
...
test = []
test_desu()
test_sorted = sorted(test, reverse=True, key=lambda x: x[1])
for i in test_sorted[0:20]:
    print(i)
